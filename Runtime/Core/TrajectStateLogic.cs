...
namespace AV.Traject.Runtime.Core
{
    ...
    public static class TrajectStateLogic
    {
        ...
        /// <summary>
        /// Main update workflow for trajectory state.
        /// Advances the trajectory state and returns TRUE if the trajectory just completed or looped this frame.
        /// </summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        [BurstCompile]
        public static bool Tick(
            in TrajectState state,
            float deltaTime,
            ...
            out bool eventTriggered)
        {
            ...
            if (!IsPlaying(in state.StatusFlags))
            {
                CalculateNormalizedProgress(
                    in state.PlaybackTimer.Current,
                    in state.PlaybackTimer.Duration,
                    out normalizedTime);
                newTime = state.PlaybackTimer.Current;
                newFlags = state.StatusFlags;
            ...
            CalculateNextTime(
                in state.PlaybackTimer.Current,
                in deltaTime,
                in state.PlaybackSpeedMultiplier,
                out newTime);

            eventTriggered = false;
            bool isLooping = IsLooping(in state.StatusFlags);
            ...
            else
            {
                ...
                {
                    // Stop playing
                    newFlags &= ~TrajectStatusFlags.IsPlaying;
                    newFlags |= TrajectStatusFlags.HasCompleted;
                    eventTriggered = true;
                }
            }
            ...
            if (IsPingPong(in newFlags))
            {
                ApplyPingPong(in normalizedTime, out normalizedTime);
            }
